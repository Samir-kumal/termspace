...
pub fn close_random_fds() {
    // Signal-safe version of closing fds to avoid deadlock during fork.
    // Iterating /dev/fd allocates memory (read_dir, Vec) which is undefined behavior
    // and causes deadlocks in multi-threaded processes like Tauri.
    // So we just close a fixed range of common FDs safely.
    for fd in 3..1024 {
        unsafe {
            libc::close(fd);
        }
